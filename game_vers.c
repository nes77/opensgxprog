...
static void print_key(sgx_keydata* ptr) {
    ...
    for(int i = 0; i < KEY_SIZE; i++) {
        ...
        sprintf(byte_hex_rep, "%02hhX", (unsigned char) ptr->aligned_ptr[i]);
...
static void sgx_keydata_init(sgx_keydata* ptr) {
    ptr->aligned_ptr = ptr->data;
    //original_ptr = (uint8_t*)((((uint64_t) ptr) & ~127) + 128);

    sigstruct_t* sigstruct;
    sigstruct = sgx_load_sigstruct("user/demo/game.conf");

    keyrequest_t keyrequest = {0};
    keyrequest.isvsvn = sigstruct->isvSvn;
    printf("%08X\n", (unsigned int) sigstruct->isvSvn);
    fflush(stdout);
    keyrequest.keypolicy.mrenclave = 0;
    keyrequest.keypolicy.mrsigner = 1;
...
// Aligns the pointer in the key struct, and initializes it with the EGETKEY instruction.
static void sgx_keydata_init_isvsvn(sgx_keydata* ptr, uint16_t isvsvn) {
    ptr->aligned_ptr = ptr->data;
    //original_ptr = (uint8_t*)((((uint64_t) ptr) & ~127) + 128);

    keyrequest_t keyrequest = {0};
    keyrequest.isvsvn = isvsvn;
    keyrequest.keypolicy.mrenclave = 0;
    keyrequest.keypolicy.mrsigner = 1;
    keyrequest.keyname = SEAL_KEY;
...
/*
static int try_decrypt(sgx_keydata* key, score_data* data, score_data* out) {
    size_t i, v_index, key_index;
    size_t bounded_i;
    uint32_t* data_v_ptr;
    uint32_t* key_part_ptr;
    ...
    return verify_score_data(out);
}

static int verify_score_data(score_data* data) {
    uint8_t checksum = hash((uint8_t*)&(data->high_score), sizeof(uint64_t));
...
void enclave_main() {
    ...
    uint16_t counter = sigstruct->isvSvn >> 8;
    ...
    if (!(byte_num == 1)) {
        ...
        while (counter && !success) {
            printf("Trying %d\n", counter);
            memset(&key, 0, sizeof(sgx_keydata));
            sgx_keydata_init_isvsvn(&key, counter << 8);
